...
function convertToSphereCoords(coordinates_array, sphere_radius) {
    var lambda = coordinates_array[0]*Math.PI/180.0;
    var phi = coordinates_array[1]*Math.PI/180.0;


    x_values.push(Math.cos(phi) * Math.cos(lambda) * sphere_radius);
    y_values.push(Math.sin(phi) *
        sphere_radius);
    z_values.push(-Math.cos(phi) * Math.sin(lambda) * sphere_radius);
}
